...
class PCPlugin:
   def input(self, filename):
    ...
    #dm = np.array(bin_data).reshape((5000, 5))
      ...
      self.dm = numpy.genfromtxt(filename,delimiter=',')
      self.dm = self.dm.astype(int)

   def run(self):
    (self.g, sep_set) = estimate_skeleton(indep_test_func=ci_test_bin,
                                     data_matrix=self.dm,
                                     alpha=0.01)
    self.g = estimate_cpdag(skel_graph=self.g, sep_set=sep_set)
